...
- 左聊天区抽屉现在新增两个 tab：**当前项目**（默认，行为不变）和**全部会话**。
- 「全部会话」模式绕过原来的 `isSameProject` 项目过滤，列出 `~/.pi/agent/sessions/` 下所有 session，并按 encoded cwd 目录分组渲染。
- 文件夹头优先显示项目名（如 `--H--github-PiDeck--` → `PiDeck`），无法匹配的项目则把 encoded 名字反解回可读的 Windows/Linux 路径。
- 文件夹按最近更新时间倒序，活动项目自动浮到最上面。
- 之前因为项目路径严格匹配而看不到的旧 `pi-desktop/chat-workspace`、`pideck-maestro/chat-workspace` 等目录下的会话现在都能找到。

### 使用方式

1. 打开左聊天区抽屉。
2. 点击头部的**全部会话**。
3. 展开任意文件夹，点击里面的 session 即可载入到主聊天区。
...
- `v0.6.6-17` 与原版 PiDeck 使用相同的 appId，NSIS 安装器可自动检测旧安装并原地升级，无需卸载，不丢数据。
- 如果你之前安装了 `v0.6.6.15`（点号版本），请先在"应用与功能"中**手动卸载 PiDeck**，再安装 `v0.6.6-17`。会话历史和设置均安全。

### 修复：Release workflow 竞态条件

- 构建任务并行上传 artifacts，统一的 `publish` 任务下载所有 artifacts 后一次性调用 `softprops/action-gh-release`。
- v0.6.6-16 的 4 个并发 job 同时尝试创建 release，导致竞态条件，release 未能成功生成。
...
兼容发行线正式以 **PiDeck-maestro** 命名，与上游 `ayuayue/PiDeck` 形成可分辨的对比。
...
1. 配置页新增独立的 permissions 子区块，覆盖 abort、表格编辑、规则编辑以及上游 `ui.custom` 兼容垫片新增的条目；尚未收敛的上游兼容点已记录为 `ISS-UI-001`。
2. 新增 Vision 执行面板，可在配置弹窗内直接查看并触发带 Vision 标签的操作，无需跳出当前页面。
3. 配置条目列表、API Manager、mcp-auth 引导、Plan discuss 入口统一使用最新 PiDeck RPC 配置桥接，并修正了若干展示规则。

### 模型重试链路与 Fast mode

1. 启用 Pi 原生重试以处理临时性模型失败，并统一各 provider 的重试请求入口，避免用户手动重试与 Agent 内部重试相互竞争。
2. UI 中真实呈现自动重试的完整状态（当前尝试次数、已尝试的兜底模型、最终耗尽原因），并避免原请求结束后重试流被强制终止。
3. 引入独立的 Fast mode（基于 `service_tier` 扩展），仅在运行中的 Agent、有效模型映射与 provider capability 都确认可用时显示；GPT 5.5 / 5.6 系列（Terra、Sol、Luna 等）已显式纳入。

### Agent、子代理与 Windows 稳定性

1. 修复会话压缩后丢失被压缩前消息的问题，压缩完成后早期上下文仍然可见。
2. 加固子代理 terminal 状态；停滞的子代理条目会冻结已显示的耗时，活动恢复后自动继续计时。
3. 加固 Windows pi CLI 发现逻辑，修正自定义 Git Bash 被误判为 WSL 的问题，让 Settings 页中的 `pi` 路径保持稳定。

### 回归覆盖

1. 新增或补充 Fast mode 生命周期、Fast mode 可见性、模型重试状态、会话补发入口绑定、运行时设置、消息分页、Windows pi CLI 发现等回归测试。
...
3. 提高历史会话启动时 `get_state` 的等待容忍度，并优化 timeout 诊断，避免大体积会话包误提示为 pi 安装或版本检测问题。

## v0.6.6-12 - 2026-07-29

### Maestro 扩展与 Skill

1. 扩展页将 `pi-maestro-flow` 作为推荐安装。
2. 减少启动等待与无谓网络请求。
3. 新增非模态 Maestro 健康提示：缺少主扩展或检测到 `pi-maestro-flow` 更新时，提供直接打开扩展页的操作入口，不打断当前会话。
4. Skill 列表现在发现已安装扩展包内的 `.pi/skills`，覆盖 Windows、WSL、Linux 与 macOS；包内 Skill 会显示为只读，无法被误改、重命名或删除。

### Agent、工具与子代理体验

1. 修复上游工具结束事件缺失或错配 `toolCallId` 时工具卡片长期停在“正在调用”的问题；在单工具或名称唯一的场景安全回退匹配，并在 Agent 回合结束时统一收敛遗留工具状态。
2. Agent 是否空闲改为依据已收敛的运行时状态判断，避免工具已完成但界面持续显示运行中，也避免不必要的延迟轮询。
3. 子代理停滞时冻结已显示的耗时，后续恢复活动后自动继续计时，便于区分真正运行与失联停滞。

### 性能与稳定性

1. 移除未启用 RPC 日志时的高频事件摘要、跨进程复制和落盘；工具开始/结束不再重复查询完整运行时状态。
2. 工具进度按 50 ms 合并刷新，降低连续工具调用和大历史会话中的 IPC、JSON 序列化和渲染开销，同时保留可见进度。
3. 增加 Maestro 健康、扩展安装、跨平台 Skill、工具状态收敛、子代理会话与会话刷新回归测试。
...
- 持久化运行时发现的子代理父子会话关系，应用重启后仍能保持会话树结构。
- 通过 teammate correlation ID 安全回填旧会话关系，避免普通 fork 会话被错误归类为子代理。

### 扩展与跨平台路径

- 本地目录扩展读取并显示自身 `package.json` 版本。
- 修复非系统盘 Windows 和原生 Linux 环境中的扩展目录路径处理。
...
- 修复非系统盘上的 Pi 扩展目录定位与 WSL 路径兼容问题。
- 修正子代理详情统计及内置扩展管理行为。
...
- 增加子代理全屏详情弹层，适配 Maestro `delegate` 任务查看。
- 增加未收尾 workflow Run 的恢复闭环，减少任务状态丢失。

## v0.6.6-5 - 2026-07-26

### 交互体验

- 增加其他会话等待输入时的跳转提示，便于快速处理待确认任务。
- 工具结果图片支持交给全局预览窗口打开。
...
- 接入 Maestro UCL 只读通道、等待输入状态和任务栏角标。
- 优化子代理面板效率、工具结构化展示和交互体验。
- 修复会话中的 system 提示始终显示在消息底部。

## v0.6.6-3 - 2026-07-26

### 子代理面板优化

- 子代理详情复用主对话的 Markdown、工具卡片和思考内容渲染。
- 修复 `toolCall/toolResult` 被直接显示为原始 JSON 的问题。

## v0.6.6-2 - 2026-07-26

### 稳定性修复

- 针对 GPT 长时间运行时可能出现的 `stream_read_error` 增加一次 Deck 级自动重试。
- 已执行工具的请求不会自动重发，避免重复产生文件修改或外部副作用。

## v0.6.5-5 - 2026-07-25

完成 v0.6.5-4 引入的子代理监控体验，并接通 teammate 子会话的实时数据链路。

### 子代理可见性

- 将主进程子代理状态通过 preload 完整转发到渲染进程面板
- 展示运行中和已完成子代理的角色、耗时、消息数、工具数及最近输出
- 支持展开查看子代理会话详情，并在标题栏提供专用面板开关
- 新建父会话启动后可发现延迟创建的子代理目录
- 支持 teammate 多层 run 目录等嵌套子会话布局
- 父工具完成后保留最后一次结构化子代理进度快照
- 增加 IPC 接线、嵌套会话发现和面板网格位置的回归测试

## v0.6.5-4 - 2026-07-24

增强工具执行可见性：所有运行中的工具现在都会显示实时运行时长和结构化进度详情，不再只显示静态折叠的副标题。

### 🎨 工具执行体验改进

- 为所有运行中的工具添加实时运行时长显示（每秒更新）
- 提取并渲染 `partialResult.details` 中的结构化进度（子代理进度、子调用、最近工具）
- 运行中的工具卡片自动展开以提高可见性，完成后自动折叠
- 将 `teammate` 加入工具图标匹配（Network 图标）和进度白名单
- 修复 bash 流式输出：`tool_execution_update` 事件现在立即刷新，长时间命令可实时看到进度
- 修复工具消息分组：assistant 完成文字输出后调用的工具现在会显示在文字下方，而不是上方

### ask-user-question 多选

- 主进程转发 RPC 提问中的 `multiSelect` 字段到桌面问答卡片
- `select` 且 `multiSelect=true` 时渲染 checkbox 多选，提交选中值的 JSON 数组

### 子代理面板（实验性）

- 新增右侧专用面板监控子代理运行
- 上方显示运行中代理，下方显示已完成代理
- 智能缓存：已完成的子代理标记为已缓存，避免重复扫描
...
基于上游 `ayuayue/PiDeck v0.6.6-beta.2` 的 Maestro 兼容发行版。

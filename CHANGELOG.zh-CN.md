...
- 首次打开扩展页即可读取并显示已安装 npm 扩展的本地版本；只有手动刷新或健康检查才访问 npm 查询更新，减少启动等待与无谓网络请求。
- 新增非模态 Maestro 健康提示：缺少主扩展或检测到 `pi-maestro-flow` 更新时，提供直接打开扩展页的操作入口，不打断当前会话。
- Skill 列表现在发现已安装扩展包内的 `.pi/skills`，覆盖 Windows、WSL、Linux 与 macOS；包内 Skill 会显示为只读，无法被误改、重命名或删除。

### Agent、工具与子代理体验

- 修复上游工具结束事件缺失或错配 `toolCallId` 时工具卡片长期停在“正在调用”的问题；在单工具或名称唯一的场景安全回退匹配，并在 Agent 回合结束时统一收敛遗留工具状态。
- Agent 是否空闲改为依据已收敛的运行时状态判断，避免工具已完成但界面持续显示运行中，也避免不必要的延迟轮询。
- 子代理停滞时冻结已显示的耗时，后续恢复活动后自动继续计时，便于区分真正运行与失联停滞。

### 性能与稳定性

- 移除未启用 RPC 日志时的高频事件摘要、跨进程复制和落盘；工具开始/结束不再重复查询完整运行时状态。
- 工具进度按 50 ms 合并刷新，降低连续工具调用和大历史会话中的 IPC、JSON 序列化和渲染开销，同时保留可见进度。
- 增加 Maestro 健康、扩展安装、跨平台 Skill、工具状态收敛、子代理会话与会话刷新回归测试。

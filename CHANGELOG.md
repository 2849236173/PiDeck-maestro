...
- Electron `appId` is kept as `com.ayuayue.pi-desktop` so existing NSIS/macOS/Linux installs upgrade in-place without requiring uninstall; only the `productName` (display name) changes to `PiDeck-maestro`.
...
- Existing users keep their session history, settings, pet config, vision JSON, etc. without any migration needed since the `userData` path stays unchanged.
...
- Added visual configuration entry points for MCP, model failover, Hooks, Skills, Web Search, and API Manager, with the latest PiDeck RPC configuration bridge.
- Added native desktop Plan confirmation, richer Maestro session state in the sub-agent panel, and Ask alias compatibility.
...
Maestro compatibility release based on upstream `ayuayue/PiDeck v0.6.6-beta.2`.
...
### Synced from upstream 0.6.6-beta.2

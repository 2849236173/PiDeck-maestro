...
pi-desktop 是一个用于管理多个 [pi](https://pi.dev) 编码 Agent 会话的桌面工作台。它负责桌面端工作流，Agent 能力仍由 pi CLI 提供。
...
[打开 GitHub Releases](https://github.com/ayuayue/pi-desktop/releases)

安装后首次启动时，pi-desktop 会尝试自动检测 `pi` 路径。如果检测失败，可以在设置里手动填写 pi 可执行文件路径。

...
const zhCN = {
  ...
  "config.retry.hint": "pi 的自动重试配置，通过指数退避策略处理供应商请求失败。修改后需重启 Agent 生效。",
  "config.retry.enabled": "启用",
  "config.retry.maxRetries": "最大重试次数",
  "config.retry.baseDelayMs": "基础延迟 (ms)",
  "config.retry.providerTitle": "供应商请求设置",
  "config.retry.timeoutMs": "请求超时 (ms)",
  "config.retry.providerMaxRetries": "SDK 级别重试",
  "config.retry.maxRetryDelayMs": "最大延期上限 (ms)",
...
const enUS: Record<TranslationKey, string> = {
  ...
  "config.retry.hint": "pi automatic retry with exponential backoff for provider requests. Restart the Agent for changes to take effect.",
  "config.retry.enabled": "Enabled",
  "config.retry.maxRetries": "Max Retries",
  "config.retry.baseDelayMs": "Base Delay (ms)",
  "config.retry.providerTitle": "Provider Request",
  "config.retry.timeoutMs": "Timeout (ms)",
  "config.retry.providerMaxRetries": "SDK Retries",
  "config.retry.maxRetryDelayMs": "Max Retry Delay (ms)",
